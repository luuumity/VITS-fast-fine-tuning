...
def train_and_evaluate(rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers):
  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader)):
    ...
    # 从这里可以看出，每个step都绘制一个图像，而eval是每100steps绘制一个。
    if rank==0:
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        # 上面存储的都是最新一期的模型文件，下面是把它存储为以step编号的模型文件，并删除旧的模型文件：
        # 真正实现在utils脚本中，其中关于删除旧模型的代码，preserved参数在utils中默认为4，但你如果输入了别的值则会覆盖默认值！
        if hps.preserved > 0:
          utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
...
def evaluate(hps, generator, eval_loader, writer_eval):
    global mark
    generator.eval()
    # 禁用梯度计算。在这里，所有的操作都不会被记录用于自动求导，也不会影响模型的参数更新。
    # 这对于推理阶段或在评估模型性能时非常有用，因为我们不需要计算梯度，只需要前向传播模型并获取输出。
    # 所以这里是不计算loss和反向传播的！反向传播的单词是backward，在前面。
    with torch.no_grad():
      # 这里是枚举了eval测试数据集的每一个条目。
      ...
      for i in range(hps.eval_size):
        x = x[i:i+1]
        x_lengths = x_lengths[i:i+1]
        speakers = speakers[i:i+1]

        # y_hat是 生成器generator infer出来的。
      ...
      print("\n --------------------------- the average mark for step "+ global_step +" is " + mark +" --------------------------- \n")
      # 我猜它会画成方波！
      ...
      #   hps.data.mel_fmax)
      # y_hat_mel = mel_spectrogram_torch(
      #   # 此操作并不会改变y_hat的值，而是返回一个新的浮点型张量。
      #   # 除非用y_hat来接收其返回值：y_hat = y_hat.squeeze(1).float()。
